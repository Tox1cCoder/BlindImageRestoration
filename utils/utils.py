...
def extract_faces_and_landmarks(images: torch.Tensor, output_size=112, mtcnn: torch.nn.Module = None,
    ...
    for i, landmarks in enumerate(landmarks_batched):
        if landmarks is None:
            ...
            affine_transformations.append(get_similarity_transform_for_cv2(landmarks[0].astype(np.float32),
                                                                           refernce_points.astype(
                                                                               np.float32) * output_size))
    affine_transformations = torch.from_numpy(np.stack(affine_transformations).astype(np.float32)).to(
    ...
    return warp_affine(fp_images, affine_transformations, dsize=(output_size, output_size)).to(
        dtype=images.dtype), invalid_indices